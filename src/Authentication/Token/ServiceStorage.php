 ...
 * ServiceStorage
 */
final class ServiceStorage implements TokenStorageInterface, CanDeleteExpiredTokens
{
    /**
     * Create a new ServiceStorage.
     *
    ...
    {
        return new static($clock, $this->storage, $this->table);
    }
    ...
    public function fetchToken(string $id): TokenInterface
    {
        ...
        $item = $this->storage->table($this->table)->find($itemId);
        ...
        try {
            $token = Token::fromJsonString($item->get('token'));
        } catch (Throwable $e) {
    ...
    public function deleteExpiredTokens(): bool
    {
        try {
            $this->storage->table($this->table)
                ->where('expires_at', '<', $this->clock->now()->getTimestamp())
                ->delete();
        } catch (QueryException $e) {
            return false;
        }
    ...
    private function createUniqueItemId(int $length): string
    {
        ...
        $item = $this->storage->table($this->table)->find($id);
